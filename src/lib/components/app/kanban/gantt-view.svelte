<script lang="ts">
    ...
    const totalDays = $derived(
        Math.max(1, Math.round((timeRange.end.getTime() - timeRange.start.getTime()) / DAY_MS)),
    );
    ...
    const columns = $derived.by(() => {
        ...
        if (zoomLevel === "day") {
            ...
            while (i < totalDays) {
                ...
                const daysLeft = Math.round((monthEnd.getTime() - d.getTime()) / DAY_MS) + 1;
    ...
    // ── Bar positioning ────────────────────────────────────────────────────
    const colWidth = $derived(zoomLevel === "day" ? 48 : zoomLevel === "week" ? 28 : 10);
    ...
    function getBarWidth(ticket: Ticket): number {
        const created = new Date(ticket.created_at).getTime();
        if (!ticket.due_date) {
            return Math.max(colWidth, ((now.getTime() - created) / DAY_MS) * colWidth);
        }
    ...
    function getDaysLeft(ticket: Ticket): string {
        ...
        const diff = Math.ceil((new Date(ticket.due_date).getTime() - now.getTime()) / DAY_MS);
    ...
    const contentHeight = $derived(
        flatRows.reduce((h, r) => h + (r.kind === "group" ? GROUP_H : ROW_H), 0),
    );

    const todayOffset = $derived(
        ((startOfDay(now).getTime() - timeRange.start.getTime()) / DAY_MS) * colWidth,
    );
    ...
    function handleBarLeave() { hoveredTicketId = null; }

    const hoveredTicket = $derived(
        hoveredTicketId ? filteredTickets.find((t) => t.id === hoveredTicketId) ?? null : null,
    );
...
<div class="gantt-shell">
    ...
    <div class="gantt-toolbar">
        <Toolbar size="sm">
            <ToolbarContent>
                <ToolbarSearch bind:value={searchQuery} placeholder="Filter tickets..." />
            </ToolbarContent>
        </Toolbar>
        <div class="gantt-zoom">
            <Button kind={zoomLevel === "day" ? "primary" : "ghost"} size="small" on:click={() => (zoomLevel = "day")}>Day</Button>
            <Button kind={zoomLevel === "week" ? "primary" : "ghost"} size="small" on:click={() => (zoomLevel = "week")}>Week</Button>
            <Button kind={zoomLevel === "month" ? "primary" : "ghost"} size="small" on:click={() => (zoomLevel = "month")}>Month</Button>
        </div>
    </div>

    {#if filteredTickets.length === 0}
        <div class="gantt-empty">
            <InlineNotification kind="info" title="No tickets"
                subtitle={searchQuery ? `No tickets match '${searchQuery}'.` : "Create tickets to see the timeline."}
                hideCloseButton />
        </div>
    {:else}
        ...
        <div class="gantt-grid">
            ...
            <div class="gantt-col-headers" bind:this={rightPanelRef}>
                <div class="gantt-col-headers-inner" style="width:{timelineWidth}px">
                    {#each columns as col}
                        <div class="gantt-col-h" style="width:{col.span * colWidth}px">
            ...
            <div class="gantt-labels" bind:this={leftPanelRef}>
                {#each flatRows as row}
                    {#if row.kind === "group"}
                        <div class="label-group" style="height:{GROUP_H}px;--accent:{row.color}">
                    ...
                    {:else}
                        <div class="label-ticket" style="height:{ROW_H}px" class:label-overdue={isOverdue(row.ticket)}>
                            <span class="label-title">{row.ticket.title}</span>
                            <span class="label-meta">{getDaysLeft(row.ticket)}</span>
                        </div>
                    {/if}
                {/each}
            </div>
            ...
            >
                <div class="gantt-canvas-inner" style="width:{timelineWidth}px;height:{contentHeight}px">
                    <!-- Grid columns -->
                    {#each { length: totalDays } as _, i}
                        {@const d = new Date(timeRange.start.getTime() + i * DAY_MS)}
                        <div
                            class="grid-col"
                            class:grid-weekend={d.getDay() === 0 || d.getDay() === 6}
                            style="left:{i * colWidth}px;width:{colWidth}px;height:{contentHeight}px"
                        ></div>
                    {/each}

                    <!-- Today marker -->
                    {#if todayOffset >= 0 && todayOffset <= timelineWidth}
                        <div class="today-line" style="left:{todayOffset}px;height:{contentHeight}px"></div>
                    {/if}

                    <!-- Rows and bars -->
                    {#each flatRows as row, idx}
                        {@const top = flatRows.slice(0, idx).reduce((h, r) => h + (r.kind === "group" ? GROUP_H : ROW_H), 0)}
                        {#if row.kind === "group"}
                            <div class="canvas-group-row" style="top:{top}px;height:{GROUP_H}px;width:{timelineWidth}px"></div>
                        {:else}
                            <div class="canvas-row" style="top:{top}px;height:{ROW_H}px;width:{timelineWidth}px">
                                ...
                                <div
                                    ...
                                    class:bar--dragging={dragTicketId === row.ticket.id}
                                    style="left:{getBarLeft(row.ticket)}px;width:{dragTicketId === row.ticket.id ? dragCurrentWidth : getBarWidth(row.ticket)}px;--bar-c:{row.color}"
                                    onmouseenter={(e) => handleBarHover(e, row.ticket.id)}
                                    onmouseleave={handleBarLeave}
                                >
                                    <div class="bar-fill" style="width:{getElapsedPercent(row.ticket)}%"></div>
    ...
    <!-- ── Tooltip ──────────────────────────────────────────────────── -->
    {#if hoveredTicket}
        <div class="gantt-tip" style="left:{tooltipX + 14}px;top:{tooltipY - 8}px">
            <strong>{hoveredTicket.title}</strong>
            <div class="tip-row">
                <span class="tip-label">Status</span>
                <Tag type="gray" size="sm">{TICKET_STATUS_CONFIG[hoveredTicket.status].label}</Tag>
            </div>
            <div class="tip-row">
                <span class="tip-label">Priority</span>
                <Tag type={TICKET_PRIORITY_CONFIG[hoveredTicket.priority]?.tagColor || "blue"} size="sm">
                    {TICKET_PRIORITY_CONFIG[hoveredTicket.priority]?.label}
                </Tag>
            </div>
            <div class="tip-row">
                <span class="tip-label">Timeline</span>
                <span class="tip-val" class:tip-overdue={isOverdue(hoveredTicket)}>{getDaysLeft(hoveredTicket)}</span>
            </div>
            {#if hoveredTicket.due_date}
                <div class="tip-row">
                    <Calendar size={12} />
                    <span class="tip-val">{new Date(hoveredTicket.due_date).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}</span>
                </div>
            {/if}
        </div>
    {/if}
...
<style>
    /* ── Shell ────────────────────────────────────────── */
    .gantt-shell { display: flex; flex-direction: column; height: 100%; background: var(--cds-ui-background); overflow: hidden; }

    /* ── Toolbar ──────────────────────────────────────── */
    .gantt-toolbar { display: flex; align-items: center; border-bottom: 1px solid var(--cds-ui-03); flex-shrink: 0; }
    .gantt-toolbar :global(.bx--toolbar) { flex: 1; }
    .gantt-zoom { display: flex; gap: 2px; padding: 0 0.75rem; flex-shrink: 0; }
    ...
    .gantt-corner {
        ...
        display: flex; align-items: flex-end; padding: 0 0.75rem 0.375rem;
        background: var(--cds-ui-01);
        border-bottom: 1px solid var(--cds-ui-03);
        border-right: 1px solid var(--cds-ui-03);
        font-size: 0.6875rem; font-weight: 600; text-transform: uppercase;
        letter-spacing: 0.06em; color: var(--cds-text-02);
        z-index: 4;
    }
    ...
    .gantt-col-headers-inner { display: flex; height: 100%; }
    .gantt-col-h {
        display: flex; flex-direction: column; align-items: center;
        justify-content: flex-end; padding-bottom: 0.25rem;
        border-right: 1px solid color-mix(in srgb, var(--cds-ui-03) 50%, transparent);
        flex-shrink: 0;
    }
    .col-main { font-size: 0.625rem; font-weight: 600; color: var(--cds-text-01); text-transform: uppercase; letter-spacing: 0.03em; }
    .col-sub { font-size: 0.5625rem; color: var(--cds-text-02); }
    ...
    .label-group {
        ...
        padding: 0 0.75rem; font-size: 0.75rem; color: var(--cds-text-01);
        background: var(--cds-ui-01);
        border-bottom: 1px solid var(--cds-ui-03);
    }
    .label-group :global(svg) { color: var(--accent); flex-shrink: 0; }
    .label-accent {
        width: 3px; height: 14px; border-radius: 2px;
        background: var(--accent); flex-shrink: 0;
    }
    .label-count {
        font-size: 0.625rem; font-weight: 600; color: var(--cds-text-02);
        background: var(--cds-ui-03); padding: 0 0.3rem; border-radius: 6px;
        min-width: 1rem; text-align: center; line-height: 1.125rem;
    }

    .label-ticket {
        display: flex; flex-direction: column; justify-content: center;
        padding: 0 0.75rem;
        border-bottom: 1px solid color-mix(in srgb, var(--cds-ui-03) 40%, transparent);
    }
    .label-title {
        font-size: 0.75rem; color: var(--cds-text-01); font-weight: 500;
        white-space: nowrap; overflow: hidden; text-overflow: ellipsis; line-height: 1.2;
    }
    .label-meta { font-size: 0.625rem; color: var(--cds-text-02); }
    .label-overdue .label-meta { color: var(--cds-support-01); font-weight: 600; }
    ...
    .gantt-canvas::-webkit-scrollbar { width: 6px; height: 6px; }
    .gantt-canvas::-webkit-scrollbar-track { background: transparent; }
    .gantt-canvas::-webkit-scrollbar-thumb { background: var(--cds-ui-04); border-radius: 3px; }
    ...
    .grid-col {
        ...
        border-right: 1px solid color-mix(in srgb, var(--cds-ui-03) 30%, transparent);
    }
    .grid-weekend { background: color-mix(in srgb, var(--cds-ui-03) 10%, transparent); }

    /* ── Today ────────────────────────────────────────── */
    .today-line {
        position: absolute; top: 0; width: 2px;
        background: var(--cds-support-01); z-index: 5; pointer-events: none;
    }
    ...
    /* ── Bars ─────────────────────────────────────────── */
    .bar {
        position: absolute; top: 4px; height: 28px; border-radius: 6px;
    ...
    .bar--done .bar-fill { background: var(--bar-c); opacity: 0.6; border-radius: 5px; }
    .bar--overdue {
    ...
    .bar--no-due { border-style: dashed; opacity: 0.6; }
    .bar-label {
        position: relative; z-index: 1; padding: 0 1rem 0 0.5rem;
        font-size: 0.6875rem; font-weight: 500; color: var(--cds-text-01);
    ...
    .gantt-tip {
        ...
        background: var(--cds-ui-01); border: 1px solid var(--cds-ui-03);
        border-radius: 6px; padding: 0.625rem 0.75rem;
        box-shadow: 0 8px 24px rgba(0,0,0,0.25);
        min-width: 180px; max-width: 280px;
        display: flex; flex-direction: column; gap: 0.375rem;
    }
    .gantt-tip strong { font-size: 0.8125rem; color: var(--cds-text-01); }
    .tip-row { display: flex; align-items: center; gap: 0.375rem; }
    .tip-label { font-size: 0.6875rem; color: var(--cds-text-02); min-width: 48px; }
    .tip-val { font-size: 0.75rem; color: var(--cds-text-01); }
    .tip-overdue { color: var(--cds-support-01) !important; font-weight: 600; }
    .tip-row :global(svg) { color: var(--cds-text-02); flex-shrink: 0; }
</style>
