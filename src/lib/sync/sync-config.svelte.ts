...
export function useSyncConfig() {
    async function load(db: Database): Promise<void> {
        try {
            const rows = await db.select<any[]>(
                `SELECT remote_url, access_token, branch, auto_sync, last_synced_at
    ...
    async function save(db: Database): Promise<void> {
        ...
        await db.execute(
            `INSERT INTO sync_config (id, remote_url, access_token, branch, auto_sync, last_synced_at, updated_at)
             VALUES (1, ?, ?, ?, ?, ?, ?)
